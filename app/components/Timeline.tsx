            ...
            <div key={index} className="relative group cursor-pointer">
              <div className="grid grid-cols-1 sm:grid-cols-[130px_1fr] gap-1 sm:gap-4 transition-transform duration-200 group-hover:-translate-y-1">
                <div className="flex items-start gap-2 pt-4">
                  ...
                  <h3 className="text-gray-900 dark:text-white font-medium text-sm">{item.company}</h3>
                </div>

                <div className="pl-4 sm:pl-0 sm:p-4">
                  <div className="flex items-start justify-between mb-1">
                    <p className="text-gray-500 dark:text-gray-400 italic text-xs">{item.role}</p>
                    <span className="text-gray-500 dark:text-gray-400 text-xs ml-4 whitespace-nowrap">{item.period}</span>
                  </div>
                  <ul className="list-disc list-inside mt-1">
                    <li className="text-gray-600 dark:text-gray-300 text-xs leading-relaxed">{item.description}</li>
                  </ul>
                </div>
              </div>
            </div>
